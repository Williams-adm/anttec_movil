...
// ✅ go_router eliminado porque se usa Navigator para el pushReplacement
...
class _FacturaScreenState extends State<FacturaScreen> {
  ...
  // ✅ Marcado como final y ahora se usa en el JSON de envío
  ...
  Future<void> _buscarRuc() async {
    ...
    try {
      ...
      if (mounted && data != null) {
        ...
        _showNotice("Empresa encontrada", Symbols.check_circle, Colors.green);
      }
    } finally {
  ...
  Future<void> _validarYFinalizar(double total, CartProvider cart) async {
    if (_rucController.text.length != 11 ||
        _razonSocialController.text.isEmpty) {
      _showNotice("Complete los datos fiscales", Symbols.domain, Colors.orange);
      return;
    }
    ...
    final orderData = {
      ...
      "document_type": _tipoDocumento, // ✅ Ahora la variable se utiliza aquí
        ...
        actions: [
          ElevatedButton.icon(
            onPressed: () {
              Navigator.pop(ctx);
              if (_pdfPath != null) {
                // ✅ Navegación corregida para que no se pueda dar "atrás"
